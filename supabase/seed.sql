...
CREATE OR REPLACE FUNCTION "public"."get_hot_rooms"() RETURNS "json"
    LANGUAGE "plpgsql" SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION "public"."increment_today_message_count"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$begin
...
CREATE TABLE IF NOT EXISTS "public"."daily_message_analysis" (
    "room" "text" NOT NULL,
    "date" "date" NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."opensea_api_errors" (
    "id" bigint NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "error_message" "text" NOT NULL
);

ALTER TABLE "public"."opensea_api_errors" OWNER TO "postgres";

ALTER TABLE "public"."opensea_api_errors" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."opensea_api_errors_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS "public"."user_nft_ownership" (
...
ALTER TABLE ONLY "public"."opensea_api_errors"
    ADD CONSTRAINT "opensea_api_errors_pkey" PRIMARY KEY ("id");
...
CREATE POLICY "insert" ON "public"."user_details" FOR INSERT WITH CHECK ((("ens" = NULL::"text") AND ("jsonb_typeof"("pfp") = 'null'::"text") AND (("auth"."jwt"() ->> 'wallet_address'::"text") = "wallet_address")));

CREATE POLICY "new message" ON "public"."chat_messages" FOR INSERT WITH CHECK (((("length"("message") > 0) OR ("jsonb_typeof"("rich") <> 'null'::"text")) AND (("auth"."jwt"() ->> 'wallet_address'::"text") = "author") AND ((NOT (POSITION((':'::"text") IN ("room")) > 0)) OR (( SELECT "user_nft_ownership"."owned"
...
ALTER TABLE "public"."opensea_api_errors" ENABLE ROW LEVEL SECURITY;

ALTER TABLE "public"."room_profiles" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "update" ON "public"."user_details" FOR UPDATE USING ((("ens" = NULL::"text") AND ("jsonb_typeof"("pfp") = 'null'::"text") AND (("auth"."jwt"() ->> 'wallet_address'::"text") = "wallet_address"))) WITH CHECK ((("ens" = NULL::"text") AND ("jsonb_typeof"("pfp") = 'null'::"text") AND (("auth"."jwt"() ->> 'wallet_address'::"text") = "wallet_address")));
...
GRANT ALL ON TABLE "public"."opensea_api_errors" TO "anon";
GRANT ALL ON TABLE "public"."opensea_api_errors" TO "authenticated";
GRANT ALL ON TABLE "public"."opensea_api_errors" TO "service_role";

GRANT ALL ON SEQUENCE "public"."opensea_api_errors_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."opensea_api_errors_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."opensea_api_errors_id_seq" TO "service_role";
